...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_SSIDS, default=False): cv.boolean,
    }
)
...
def get_scanner(hass, config):
    ...
    include_ssids = info[CONF_SSIDS]

    api = Netgear(password, host, username, port, ssl)
    scanner = NetgearDeviceScanner(api, devices, excluded_devices, accesspoints, include_ssids)
...
class NetgearDeviceScanner(DeviceScanner):
    ...
    def __init__(
        ...
        self.include_ssids = include_ssids
        self.last_results = []
        self._api = api
    ...
    def scan_devices(self):
        ...
        if self.include_ssids:
          ssids = list(set([device.ssid for device in self.last_results]))
          for ssid in ssids:
              if ssid is not None:
                  devices.append(ssid)
    ...
    def get_attached_devices(self):
        ...
        if self.tracked_accesspoints or self.include_ssids:
